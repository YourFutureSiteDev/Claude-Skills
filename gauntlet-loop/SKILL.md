...
## Bars for Your Future Site work

Local additions to this skill. Everything above is unmodified from the original.

When the goal is one of Byron's recurring jobs, these are the bars that pass the
named / fetchable / comparable test. Offer them first before inventing new ones.

| Job | Bar that works |
|---|---|
| A client website for a trade (sparky, plumber, mechanic, buggy repair) | The live site of a named Australian trade business that already ranks page one for that service plus suburb. Screenshot desktop and mobile. |
| A prospect mockup sent cold | The prospect's own current site, if they have one. Ours has to visibly beat the thing they already paid for. |
| An outreach email or SMS | Three real emails from a named agency that Byron would open himself. Blind test: which one gets a reply. |
| A landing page or storefront | The live pricing page of a named competitor in the same market, at the same price point. |
| A Discord bot flow or console page | A named shipped product's equivalent screen, run in a browser, not a description of it. |

Two Byron-specific rules:

- **Prices, dates and names never go in the loop.** The critic will happily
  "improve" A$500 into A$5,000 or invent a client. Pin every real figure in the
  prompt as fixed and tell the critic those are not its business.
- **A real client site never gets published by the loop.** The loop builds and
  judges. Byron publishes. Say so in the prompt when the goal touches a live site.

## Credit and licence

Skill by Jay E at RoboNuggets (https://github.com/robonuggets/gauntlet-loop),
CC BY 4.0. Technique by Matt Shumer (https://github.com/mshumer), who wrote the
original prompt while building Claude of Duty. Installed 3 Sep 2026. The only
change is this final section; the skill body above is the original text.
