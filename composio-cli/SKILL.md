...
## On this machine: run it through WSL

The Composio CLI has no Windows build, so it lives inside WSL Ubuntu as the
user `byron`. Every `composio ...` command below must be prefixed:

    MSYS_NO_PATHCONV=1 wsl.exe -d Ubuntu --user byron -- bash -lc 'composio ...'

Two gotchas, both of which will waste your time otherwise:

- `MSYS_NO_PATHCONV=1` is required. Without it Git Bash rewrites `/bin/sh` and
  other POSIX paths into Windows paths and the command dies with "No such file".
- Shell variables do not survive the nesting. Use literal paths inside the
  quoted command rather than `$VAR`, or the expansion silently becomes empty.
- Output sometimes comes back empty when piped directly. Redirect to a file
  inside WSL first, then `cat` it in the same command.

The Windows filesystem is visible from inside WSL at `/mnt/c/...`.
